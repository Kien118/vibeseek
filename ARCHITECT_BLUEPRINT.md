...
- **2026-04-19 (🎉 PHASE 3 COMPLETE)** — All 5 Phase 3 tasks done: T-301 (pgvector + `card_embeddings` + `raw_text` col + quiz UNIQUE sync), T-302 (`embeddings.ts` + `/api/embeddings/ensure` lazy idempotent), T-303 (`chat.ts` RAG retrieve + streaming Gemini→Groq fallback), T-304 (`/api/chat` SSE route + in-memory rate limit 10/min/anon), T-305 (ChatPanel + `/chat/[documentId]` + dashboard link + localStorage history). Merge commits: `d630f8e` / `a9cc2d5` / `7e7ea3a` / `1c023f7` / `08c803a`. **1 hotfix total vs Phase 2 baseline 8 hotfixes** — 7-step pipeline (failure-modes-section in spec, user-runnable test plan in spec, architect local-run during review, three-strikes circuit breaker, E2E formal batch step) validated end-to-end. T-305 hotfix (`49f628d`) fixed 2 E2E bugs: (a) input text invisible — globals.css `body{color:white}` inherited by input without explicit class → fix: `text-gray-900 bg-white placeholder:text-gray-400`; (b) localStorage wiped on reload — mount-time race between load + persist useEffect with initial `messages=[]` closure → fix: `saveHistory` contract change "only persists non-empty, use `clearHistory` for intentional clears". Decision D-1 (Phase 3): embedding model swap `text-embedding-004` → `gemini-embedding-001` + `outputDimensionality:768` (SDK compat). Process notes: architect self-inflicted incident Phase 3 internal — running `npm run build` during user's active dev server corrupted `.next/server/` cache, logged as "DO NOT build during dev". Next: E2E full Phase 3 smoke OR Phase 4 (P-401..P-405 video quality polish).
