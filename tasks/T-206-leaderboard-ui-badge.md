...
## Acceptance criteria
- [ ] AC-1: 5 file mới + 1 sửa (`layout.tsx`).
- [ ] AC-2: `npx tsc --noEmit` + `npm run build` pass.
- [ ] AC-3: Vào `/` (landing) → **KHÔNG** thấy Vibe Points badge.
- [ ] AC-4: Vào `/dashboard`, `/leaderboard`, `/quiz/<id>` → thấy badge top-right, hiển thị điểm hiện tại.
- [ ] AC-5: Click badge → navigate `/leaderboard`.
- [ ] AC-6: `/leaderboard` hiển thị top 20 (sau khi ≥ 1 user có điểm từ T-205 test), user hiện tại được highlight nếu có trong top + show " (bạn)".
- [ ] AC-7: Đổi tên ở input → save → leaderboard update display_name.
- [ ] AC-8: Empty state (DB trống) → bảng render message "Chưa có ai ghi điểm..."
- [ ] AC-9: Mobile responsive — cột "Đúng" ẩn < sm, table không tràn.
